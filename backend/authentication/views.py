...
def redirect_login_view(request):
    return redirect('auth:login',request=request)

@exclude_middleware
def login_view(request):
    if request.user.is_authenticated :
        # Get the 'next' parameter from the query string
        next_url = request.GET.get('next', f"/master/")
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if user is not None:
                login(request, user)
                # Get the 'next' parameter from the query string
                next_url = request.POST.get('next', f"/master/")
                return redirect(next_url)
            else:
    ...
    if request.method == 'GET':
        ...
        try:
            ...
            if password1 == password2:
                ...
                return redirect('auth:logout',request=request)
            else:
    ...
    if request.method == 'GET':
        ...
        try:
            ...
            if user and user.email:
                subject = 'Trackmythests-Password Reset Link'
